...
void add_task(){
    ...
    task[strcspn(task, "\n")] = '\0';
    fptr1 = fopen("Tasks.txt", "a");
    if (fptr1 == NULL) {
        printf("Error: Could not open Tasks.txt for writing.\n");
        return;
    }
    fprintf(fptr1, "%s\n", task);
    fclose(fptr1);
    fptr2 = fopen("Time.txt", "a");
    if (fptr2 == NULL) {
        printf("Error: Could not open Time.txt for writing.\n");
        return;
    }
    fprintf(fptr2, "%d\n", 0);
    fclose(fptr2);
    printf("\nTask '%s' successfully added!\n", task);
}
...
int timer(int minutes) {
    //Comments included for better understanding
...
void update_time(int time,int position){
    ...
    if (rptr == NULL || wptr == NULL) {
        printf("Error: File access denied during time update.\n");
        return;
    }
    ...
    if (flag){
        ...
        while(fgets(writeline,50,fptr1)!=NULL && fgets(writetime,20,fptr3)!=NULL){
            ...
            if(filecounter!=counter){
                fprintf(fptr2,"%s",writeline);
                fprintf(fptr4,"%s",writetime);
            }
            else{
                writeline[strcspn(writeline, "\n")] = 0;
                writetime[strcspn(writetime, "\n")] = 0;
                fprintf(fptr5,"Task:%s\n",writeline);
                fprintf(fptr5,"Time Spent:%s\n",writetime);
                fprintf(fptr5,"--------------------------\n");
            }
        }
    ...
    do {
        ...
        switch (choose) {
            case 1:
                ...
                printf("6. Do not Add multiple tasks with same name, that will cause errors(Not Case Sensitive)");
